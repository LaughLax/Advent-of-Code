...
class IntCode:
    ...
    def run_op(self):
        ...
        if op == 1:
            ...
            while self.input_pos > len(self.auto_inputs) - 1:
                self.auto_inputs.extend([ord(c) for c in input()])
                self.auto_inputs.append(10)
            self.set_addr(params[0], self.auto_inputs[self.input_pos])
            self.input_pos += 1
        elif op == 4:
            # sys.stdout.write(chr(params[0]))
...
class AdventOfCode:
    ...
    def __init__(self, filename):
        ...
        self.machine = IntCode(f'droid', self.init_cond)

    def part1(self):
        self.machine.reset_state()
        self.machine.auto_inputs = [c for c in b'''east
take sand
west
west
north
...
''']
        ...
        my_str = ''.join(map(chr, self.machine.outputs)).splitlines()[-1]
        return my_str
